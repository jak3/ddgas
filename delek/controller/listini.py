...
def list_prodotti(id_produttore):
    ...
    return render_template('listini/list.html',
                           prodotti=get_prodotti(id_produttore),
                           produttore=get_produttore(
                               id_produttore, column_names=['id', 'nome']))
...
def create(id_produttore):
    ...
    return render_template('listini/create.html',
                           categorie=get_categorie(id_produttore))
...
def update(id_produttore):
    ...
    if request.method == 'POST':
        ...
        if error:
            ...
            return render_template('listini/list-mod.html',
                                   prodotti=get_prodotti(id_produttore),
                                   produttore=get_produttore(
                                       id_produttore,
...
def esporta(id_produttore):
    ...
    get_db().copy_to(content, 'listino_{0}'.format(id_produttore),
                     sep=',', columns=['categoria', 'disponibile',
                                       'descrizione_prodotto', 'dettaglio_qta',
                                       'prezzo', 'n_min_colli', 'n_max_colli',
                                       'colli_disponibili', 'nota'])

    content.seek(0)
    return send_file(content,
                     as_attachment=True,
                     download_name='%s.csv' % fname,
                     mimetype='text/csv'
                     )
...
def scarica_template(id_produttore):
    ...
    content = BytesIO(bytes(','.join([
        'categoria', 'disponibile', 'descrizione prodotto',
        'dettaglio quantità', 'prezzo', 'n minimo colli', 'n massimo colli',
        'colli disponibili', 'nota']), 'utf8'))
    return send_file(content,
                     as_attachment=True,
...
def importa(id_produttore):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for row in crr:
                if len(row) != 9:
                    flash('Le colonne alla riga (%s) non corrispondono' % row,
                          'warning')
                    return redirect(request.url)
                ...
                dbi.execute(
                    ...
                    [row[0], bool(row[1])if row[1] else True,
                     row[2], row[3],
                     float(row[4]),
            ...
            flash('Sono stati riscontrati errori nella lettura del file',
                  'warning')
            return redirect(request.url)
...
def delete(id_produttore, id_prodotto):
    ...
    get_db().execute(
        'DELETE FROM listino_{idl} WHERE id = %s'.format(
            idl=id_produttore),
        (id_prodotto,))
