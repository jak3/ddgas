...
from delek.controller.auth import (
    get_utente_by_id, get_utenti, login_required, is_ruolo, is_mod_or_ref_of
)
...
def list_ordini():
    ...
    mie_prossime_consegne = sorted(list(
        filter(lambda o: o['id_produttore'] in da_chi_ordino,
               prossime_consegne)),
        key=lambda o: o['consegna'])
...
def chiusi():
    ...
    return render_template('ordini/chiusi.html',
...
def esporta():
    ...
    writer = csv.DictWriter(
        content, fieldnames=pagamenti_ordini_chiusi[0].keys())
    writer.writeheader()
    ...
    return send_file(mem,
                     as_attachment=True,
                     download_name='ordini-chiusi.csv',
                     mimetype='text/csv'
                     )
...
def list_archivio():
    ...
    return render_template('ordini/archivio.html',
                           ordini_archivio=_get_archivio_ordini())
...
def effettua_ordine(id_produttore):
    ...
    if request.method == 'POST':
        error = (check_inputs_ordine(request.form) or
                 _check_inputs_vincoli(id_produttore, request.form))
        ...
        if not error:
            ...
            for (id_prodotto, colli_richiesti, specifica) in filter(
                    lambda row: int(row[1] if row[1] else 0) > 0, zip(
                        request.form.getlist('id_prodotto'),
                        request.form.getlist('colli_richiesti'),
                        request.form.getlist('specifica')
                    )):
                dbi.execute(
                    'SELECT colli_disponibili FROM listino_{0} WHERE id = %s'
                    .format(id_produttore), (id_prodotto,))
                ...
                # base alla richiesta
                if disponibili > 0:
                    ordini = _get_totale_ordinati(id_produttore, id_prodotto,
                                                  notmine=True)
                    if ordini + int(colli_richiesti) > disponibili:
                        msg = ' '.join([
                            '(ordinati', str(disponibili - ordini),
                            'al posto di', colli_richiesti,
                ...
                if int(colli_richiesti) > 0:
                    dbi.execute("""
                        ...
                        (%s, %s, %s, %s)""".format(id_produttore),
                                (g.user['id'], id_prodotto, colli_richiesti,
                                 specifica))

            msg += ' '.join(['·', 'Totale Provvisorio:',
                            '%.2f' % (get_totale_prodotti_ordinati(
                                get_prodotti_ordinati(id_produttore,
                                                      g.user['id']))),
                             '€'])

            flash('Aggiornamento Ordine avvenuto con Successo ' + msg,
                  'success')

    if error:
        flash(error['error_msg'], 'warning')

    return render_template('ordini/effettua.html',
                           date_ordine=get_date_ordine(id_produttore),
                           minimo_ordine=get_minimo_ordine(id_produttore),
                           prodotti=get_prodotti_ordinati_da_tutti(
                               id_produttore, g.user['id']),
                           prodotti_ordinati=get_prodotti_ordinati(
                               id_produttore, g.user['id']),
    ...
    if request.method == 'POST':
        ...
        try:
            if any(datetime.now() > d for d in [
                ...
                error = {'error_msg':
                         'Non è consentito impostare date nel passato'}
        except ValueError:
            error = {'error_msg': u'Formato date non conforme'}
        ...
        if not error:
            ...
            dbi.execute(
                'SELECT * FROM listino_{0}'.format(
                    int(request.form['id_produttore']))
            )

            if dbi.rowcount <= 0:
                flash('Necessario prima di aprire un ordine, creare un'
                      ' listino. Comincia aggiungendo un prodotto', 'warning')
                return redirect(
                    url_for('listini.create',
            ...
            dbi.execute(
                ...
                VALUES ({placeholders})
            """.format(
                    column_names=str(tuple(cn for cn in inputs)
                                     ).replace('\'', ''),
...
def close(id_produttore):
    ...
    if 'conferma' in request.form:
        ...
        if (float(dettaglio_ordine['minimo_ordine']) <
                sum(get_spesa_totale_utenti(id_produttore).values())):
            dbi.execute(
    ...
    if 'pre-conferma' in request.form:
        flash('Sicuro di voler chiudere l\' ordine? I gasisti non potranno più'
...
def delete(id_produttore):
    ...
    if 'pre-conferma' in request.form:
        flash('Sicuro di voler annullare e cancellare TUTTI gli ordini? Non'
              ' si torna più indietro se si conferma nuovamente.', 'warning')
...
def rettifica_ordine(id_produttore):
    ...
    if request.method == 'POST':
        ...
        if not check_inputs_rettifiche(request.form):
            ...
            nuovi_totali = [float(rettifica)
                            if rettifica and float(rettifica) > 0
                            else
                            float(ordini[i]['totale_utente'])
            ...
            if any(v <= 0 for v in nuovi_totali):
                flash("""
                 ...
                 Non verranno aggiunti ai movimenti proseguendo!""", 'warning')

            if 'pre-conferma' in request.form:
                flash('Sicuro di voler confermare i totali? Ricontrollali, non'
            ...
            if 'conferma' in request.form:
                ...
                if nuovi_totali:
                    ...
                    for (i, importo) in enumerate(nuovi_totali):
                        _inserisci_rettifica(
                            ...
                            id_produttore, importo, motivazioni[i],
                            consegna)
                    for extra in extras:
                        _inserisci_rettifica(extra['id_utente'], id_produttore,
                                             extra['importo'],
                                             extra['descrizione'],
                                             consegna)
                    dbi.execute("""
                ...
                flash('Rettifiche Ordine avvenute con successo, Ordine \
                        Completato', 'success')
                return redirect(url_for('stampa.storico_tiny',
                                id_produttore=id_produttore))

            return render_template('ordini/rettifica.html',
                                   extras=extras,
                                   importi_rettifica=request.form.getlist(
                                       'importo_rettifica'),
                                   ...
                                   produttore=get_produttore(id_produttore),
                                   totale_ordine=sum(
                                       [o['totale_utente'] for o in ordini]),
                                   sicuro=sicuro,
                                   utenti=get_utenti(vincolo=("attivo",
                                                              True)),
                                   )
    ...
    return render_template('ordini/rettifica.html',
                           ...
                           ordini=ordini,
                           produttore=get_produttore(id_produttore),
                           totale_ordine=sum([o['totale_utente']
                                             for o in ordini]),
                           sicuro=sicuro,
                           utenti=filter(lambda u: u['id'] not in
                                         set(o['id_utente'] for o in ordini),
                                         get_utenti(vincolo=("attivo",
                                                             True))),
                           )
...
def rimuovi_singolo_ordine(id_produttore, id_utente):
    ...
    get_db().execute(
        """
        DELETE FROM ordine_in_corso_{0} WHERE id_utente = %s
        """.format(id_produttore), [id_utente])
    return redirect(url_for('ordini.rettifica_ordine',
                            id_produttore=id_produttore))
