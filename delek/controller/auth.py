...
from delek.model.checks import (
    check_inputs_utente, check_inputs_isid, check_inputs_quota,
    check_inputs_smemo
)
...
def utente_filter(utente: dict):
    ...
    return ' '.join(filter(None, [
        ...
        ' (', utente['username'], ')',
        '· ' + utente.get('cf').upper()
        if utente.get('cf', '') else '',
        '· ' + utente.get('email')
        if utente.get('email', '') else '',
        '· ' + utente.get('telefono')
        if utente.get('telefono', '') else '']))
    ...
    def is_ruolo_inner(view):
        ...
        def _(*args, **kwargs):
            ...
            if len(set(what).intersection(set(g.ruoli))) <= 0:
                abort(403)
            if ('moderatore' not in what and
                'referente' in what and 'id_produttore' in kwargs and
               kwargs['id_produttore'] not in g.referenze):
                abort(403)
            return view(*args, **kwargs)
        return _
...
def get_utente(vincolo=(1, 1)):
    ...
    vincolo = (AsIs(vincolo[0]) if type(vincolo[0]) is str else vincolo[0],
               vincolo[1])
    get_db().execute('SELECT * FROM utenti WHERE %s = %s',
                     vincolo)
    ...
    if request.method == 'POST':
        ...
        if not error:
            if get_utente_by_username(request.form['username']) is not None:
                ...
                dbi.execute('INSERT INTO nuovi_utenti (id_utente) VALUES (%s)',
                            (dbi.fetchone()['id'],)
                            )
                return render_template('auth/attivazione.html',
    ...
    if request.method == 'POST':
        ...
        if not error:
            ...
            if dbi.fetchone():
                ...
                reset_link = ''.join([
                    request.host_url, url_for('auth.reset_password'),
                    '?token=', token
                ])
                msg = Mail(
                    ...
                    plain_text_content=''.join([
                        ...
                        ' reset della tua password ( ', username, ' ).\n',
                        reset_link
                    ])
                )
                SendGridAPIClient(
...
def is_tesserato(id_utente, giorni_esenzione=60):
    ...
    if campagna:
        ...
        """, {'id': id_utente, 'data':
              date(campagna['data_inizio'].year,
...
COLONNE_UTENTE_MODIFICABILI = ('username', 'password', 'email', 'telefono',
                               'nome', 'cognome', 'cf')
...
def _update_user(inputs: dict, id_utente):
    ...
    if inputs.get('password'):  # se voglio cambiare password
        ...
        inputs.update({'password':
                       generate_password_hash(inputs.get('password'))})
    else:
        inputs.pop('password')
        error = check_inputs_utente(inputs)
    ...
    inputs = {k: v for k, v in inputs.items()
             if k in COLONNE_UTENTE_MODIFICABILI}
...
def toggle_attivo(id_utente):
    ...
    get_db().execute('UPDATE utenti SET attivo = NOT attivo WHERE id = %s',
                     (id_utente,))
...
def gestione_permessi(id_utente):
    ...
    if request.method == 'POST':
        ...
        if not error:
            ...
            return redirect(url_for('auth.gestione_permessi',
                                    id_utente=id_utente))
    ...
    return render_template('auth/gestione_permessi.html',
                           utente=utente, ruoli=ruoli,
                           ruoli_utente=ruoli_utente, referenze=referenze,
...
@bp.route('/remove/reference/<int:id_produttore>/<int:id_utente>',
         methods=('POST',))
@login_required
@is_ruolo(['moderatore'])
...
def accoglienza():
    ...
    return render_template('auth/accoglienza.html',
                           nuovi=get_nuovi_utenti())
    ...
    if request.method == 'POST':
        ...
        if any(a in request.form for a in ['allin', 'deleteall']):
            ...
            for id_nuovo_utente in request.form.getlist('checkbox'):
                ...
                dbi.execute(
                    'DELETE FROM nuovi_utenti WHERE id_utente = %s',
                    (id_nuovo_utente,)
                )
...
def reset_password():
    ...
    if request.method == 'POST':
        ...
        if new_password == request.form['password-check']:
            try:
                token = jwt.decode(token_encoded,
                                   current_app.config['SECRET_KEY'],
                                   algorithms=["HS256"])
                ...
                flash('Link di ripristino non valido, ripetere la'
                      ' procedura.', 'warning')
        else:
...
def tesseramenti():
    ...
    dbi.execute(
        'SELECT id, data_inizio FROM campagne_tesseramenti'
        ' ORDER BY data_inizio DESC')

    return render_template('auth/tesseramenti.html',
                           utenti=utenti,
...
def _insert_movimento(movimento: dict):
    get_db().execute("""
        ...
        """.format(column_names=str(tuple(cn for cn in movimento)
                                    ).replace('\'', ''),
