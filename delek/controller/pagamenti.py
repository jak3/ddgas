        ...
        id_movimento = insert_movimento(
            {
                ...
                'descrizione': '[Ricarica {0}] Versamento automatico'.format(provider),
...
def ricarica():
    ...
    if request.method == 'POST':
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            success_url=url_for('pagamenti.ricarica_completata', _external=True),
            cancel_url=url_for('pagamenti.ricarica', _external=True),
        )
        registra_pagamento_creato('stripe', checkout_session.id, g.user['id'], importo)
...
def ricarica_completata():
    ...
    flash(
        'Pagamento in elaborazione: il credito comparirà a breve sul tuo'
        ' conto se avvenuto correttamente.',
        'success',
    )
...
def webhook_stripe():
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        annulla_pagamento(
            'stripe', event['data']['object']['id'], 'sessione scaduta senza pagamento'
        )
        ...
        if charge.get('refunded'):
            ...
            if sessioni.data:
                annulla_pagamento('stripe', sessioni.data[0].id, 'rimborso Stripe')
...
def riconciliazione_sincronizza():
    ...
    if sessione.payment_status == 'paid':
        if conferma_pagamento('stripe', provider_ref):
            flash(
                'Ricarica {0} accreditata manualmente'.format(provider_ref), 'success'
            )
        else:
            flash(
                'Ricarica {0} già gestita o non tracciata'.format(provider_ref),
                'warning',
            )
    elif sessione.status == 'expired':
        if annulla_pagamento(
            'stripe', provider_ref, 'sessione scaduta (sincronizzazione manuale)'
        ):
            flash(
                'Ricarica {0} marcata come non riuscita'.format(provider_ref), 'success'
            )
        else:
            flash(
                'Ricarica {0} già gestita o non tracciata'.format(provider_ref),
        ...
        flash(
            'Ricarica {0} ancora in corso lato Stripe, nessuna azione'.format(
                provider_ref
            ),
            'warning',
        )
...
def ricarica_satispay():
    ...
    callback_url = (
        url_for('pagamenti.webhook_satispay', _external=True) + '?payment_id={uuid}'
    )
    body = json.dumps(
        {
            'flow': 'MATCH_CODE',
            ...
            'redirect_url': url_for('pagamenti.ricarica_completata', _external=True),
        }
    ).encode()
    pagamento = _satispay_request('POST', '/payments', body)

    registra_pagamento_creato('satispay', pagamento['id'], g.user['id'], importo)
...
def webhook_satispay():
    ...
    try:
        pagamento = _satispay_request('GET', '/payments/{0}'.format(payment_id))
