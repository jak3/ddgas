...
def get_spesa_totale_in_ordinazione():
    ...
    id_produttori_in_lavorazione = [
        dettaglio['id_produttore'] for dettaglio in dbi.fetchall()
    ]
    totale = 0
...
def get_totale_utente_temporaneo():
    ...
    return float(get_totale_utente(g.user['id'])) - get_spesa_totale_in_ordinazione()
