...
from delek.controller.auth import (
    login_required, is_ruolo, get_utenti, g, is_mod_or_ref_of
)
...
COLONNE_PRODUTTORE_MODIFICABILI = ('nome', 'email', 'telefono', 'website',
                                   'mask_mesi_consegna',
                                   'prodotto_principale', 'descrizione',
                                   'attivo')
...
def get_produttore(id_produttore, column_names=None):
    ...
    get_db().execute('SELECT {0} FROM produttori WHERE id = %s'.format(
        ','.join(column_names) if column_names else '*'), [id_produttore])
...
def list_produttori():
    ...
    referenti = {produttore['id']: get_referenti(produttore['id'])
                 for produttore in produttori}

    return render_template('produttori/list.html',
...
def create():
    ...
    return render_template('produttori/create.html', utenti=get_utenti(),
                           mesi=MESI)
...
def update(id_produttore):
    ...
    if request.method == 'POST':
        ...
        if not error:
            ...
            return redirect(url_for('produttori.update',
                                    id_produttore=id_produttore))

        flash(error['error_msg'], 'warning')

    return render_template('produttori/update.html', produttore=produttore,
                           utenti=utenti, referenti=referenti, mesi=MESI,
                           mask_mesi=mask_mesi)
...
def toggle_attivo(id_produttore):
    ...
    get_db().execute('UPDATE produttori SET attivo = NOT attivo WHERE id = %s',
                     (id_produttore,))
    return redirect(url_for('produttori.list_produttori'))
...
def storico_all(id_produttore):
    ...
    if consegne:
        return render_template('produttori/storico_all.html',
                               id_produttore=id_produttore,
                               consegne=consegne,
                               data=data,
                               ordini=ordini,
...
def storico_utente(id_storico):
    ...
    if ordine['id_utente'] != g.user['id']:
        flash('Spiacente non hai i permessi di vedere questo storico',
              'warning')
        return redirect(url_for('ordini.list_ordini'))
...
def _create_referente(id_produttore, id_utente):
    ...
    dbi = get_db()
    dbi.execute(""" INSERT INTO referenze (id_produttore, id_utente)
    ...
    dbi.execute(""" INSERT INTO arruolati (id_ruolo, id_utente)
                    VALUES (%s, %s) ON CONFLICT DO NOTHING """,
                (2, id_utente))
