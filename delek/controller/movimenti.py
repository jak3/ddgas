...
from delek.model.checks import (
    check_inputs_movimento,
    check_valid_date,
    check_aggiusta_ordine_chiuso,
)
    ...
    return '\\n'.join(
        filter(
            ...
            [
                ...
                (
                    ' '.join([' (', movimento['produttore'], ')'])
                    if movimento.get('produttore', '') or ''
                    else ''
                ),
            ],
        )
...
def is_in_args_or_form(what):
    ...
    return any(what in where for where in [request.args, request.form.keys()] if where)
...
def get_totale(id_utente=''):
    ...
    get_db().execute(
        ...
        (AsIs('per_id_utente'), id_utente) if id_utente else (1, 1),
    )
    ...
    if not error:
        ...
        if int(inputs['tipologia']) in [
            ...
            if int(inputs['tipologia']) == tipologie['giroconto']:
                ...
                inputs |= {
                    'descrizione': inputs['descrizione']
        ...
        if (
            int(inputs['tipologia']) == tipologie['giroconto']
            and inputs['per_id_utente'] == inputs['verso_id']
        ):
            return {'error_msg': u'Un giroconto necessita di due utenti diversi'}
...
def giroconto_utente():
    ...
    """
    if request.method == 'POST':
        inputs = dict(request.form) | {'tipologia': 3, 'per_id_utente': g.user['id']}
        ...
        if '- - -' in inputs.get('verso_id', '- - -'):
            error = {
                'error_msg': "Selezionare l'utente verso cui effettuare il" ' giroconto'
            }
        else:
            dauser = get_utente_by_id(inputs['per_id_utente'])['username']
            auser = get_utente_by_id(inputs['verso_id'])['username']
            inputs['descrizione'] = (
                f'[Giroconto da {dauser} verso {auser}] ' + inputs['descrizione']
            )
            ...
            if not error:
                ...
                if saldo < float(inputs['importo']):
                    error = {
                        'error_msg': 'Non presenti abbastanza soldi nel portafoglio.'
            ...
            if not error:
                with atomic():
                    ...
                    insert_movimento(
                        inputs | {'importo': -1 * float(inputs['importo'])}
                    )
...
def _list_movimenti(query, id_utente=None, own_only=False):
    ...
    if request.method == 'POST':
        ...
        if (
            not own_only
            and 'id_utente' in request.form
            and '- - -' not in request.form['id_utente']
        ):
            error = error or check_inputs_movimento(
                MultiDict({'per_id_utente': request.form['id_utente']})
            )
            filtro += ['utenti.id = ' + request.form['id_utente']]
            id_utente = int(request.form['id_utente'])

        if 'tipologia' in request.form and '- - -' not in request.form['tipologia']:
            error = error or (
                {}
                if request.form['tipologia'].isdigit()
                else {'error_msg': 'Tipologia non conforme'}
            )
            filtro += ['tipologie_movimenti.id = ' + request.form['tipologia']]

    query_params = (
        (id_utente, AsIs(' AND '.join([''] + filtro)))
        if id_utente
        else [AsIs(' AND '.join(["tipologie_movimenti.nome != 'acquisto'"] + filtro))]
    )
...
def list_movimenti_all():
    ...
    if 'id_utente' in request.form and '- - -' not in request.form['id_utente']:
        dbi.execute(
            'SELECT sum(importo) as totale' ' FROM movimenti WHERE per_id_utente = %s',
...
def totali_conti_utenti():
    ...
    return render_template(
        'movimenti/totali_conti_utenti.html', totali=_totali_conti_utenti()
    )
...
def rettifica_utente():
    ...
    return render_template('movimenti/rettifica.html', utenti=get_utenti())
    ...
    if not error:
        ...
        nuovo_totale = float(
            sum(
                [
                    o['importo']
                    for o in get_storico_ordini(
                        id_produttore, da_form(request.form['data'])
                    )
                ]
            )
        ...
        """,
            (
                ...
                nuovo_totale,
                request.form['data'],
                json.dumps(request.form['descrizione']),
                id_movimento,
            ),
        )

        flash('Nuovo Totale Aggiornato. Differenza sommata alla cassa', 'success')
        return redirect(
            url_for('produttori.list_produttori', id_produttore=id_produttore)
        )
