...
CeDescrizione = {
    'descrizione': create_checknerror(MULTI_LINE_STRING, 'Descrizione non conforme')
}
...
def gen_cenome(regex, could_be_empty=True):
    ...
    return {'nome': create_checknerror(regex, 'Nome non conforme', could_be_empty)}


CeIdProduttoreRequired = gen_ceid(
    'id_produttore', 'Id Produttore non conforme', could_be_empty=False
)
CeIdUtenteRequired = gen_ceid(
    'id_utente', 'Id Utente non conforme', could_be_empty=False
)


def gen_categoria(name, could_be_empty=True):
    """Genera una CeCategoria con nome alternavivo"""
    return name, create_checknerror(
        SINGLE_LINE_STRING, 'Categoria non conforme', could_be_empty
    )


CeNota = {'nota': create_checknerror(MULTI_LINE_STRING, 'Nota non conforme')}
CeTelefono = {'telefono': create_checknerror(PHONE, 'Numero di telefono non conforme')}
    ...
    for what, checker in checks.items():
        ...
        if isinstance(check_object, list):
            ...
            if not checker.could_be_empty and not check_object:
                return {
                    'error_msg': "Il campo '" + what + "' non può essere vuoto",
                    'id': '0',
...
def check_inputs_listino(inputs):
    ...
    checks = (
        gen_ceid('id_prodotto', 'Id prodotto non conforme')
        | {
        ...
        | dict(
            [
                gen_categoria(name)
                for name in ['categoria', 'old_categoria', 'new_categoria']
            ]
        )
    )
...
def check_inputs_produttore(inputs):
    ...
    checks = (
        gen_ceid('id_utente', 'Id Utente Referente non conforme')
        | CeId
        | gen_cenome(SINGLE_LINE_STRING)
        | CeEmail
        | CeTelefono
        | {
            'website': create_checknerror(URL, 'Pagina Web non valida'),
            ...
            'prodotto_principale': create_checknerror(
                SINGLE_LINE_STRING, 'Prodotto Principale non conforme'
            ),
            'descrizione': create_checknerror(
                MULTI_LINE_STRING, 'Descrizione non conforme'
            ),
        }
        | CeAttivo
    )
...
def check_inputs_movimento(inputs):
    ...
    checks = {
        'per_id_utente': create_checknerror(
            ID,
            'Id utente di riferimento non conforme',
        ),
        'verso_id': create_checknerror(
            ID, 'Id utente verso cui effettuare il movimento non conforme'
        ),
        # '0' = applicativo
        'gestore': create_checknerror(
            INTEGER_POSITIVE,
            'Gestore del movimento non conforme',
        ),
        'tipologia': create_checknerror(
            '(1|2|3|4|5|6|7|8)',
        ...
        'importo': create_checknerror(
            DECIMAL_MAX_TWO_PLACES_SIGNED,
...
def check_inputs_ordine(inputs):
    ...
    checks = gen_ceid(
        'id_prodotto', 'Id prodotto non conforme', could_be_empty=False
    ) | {
        'prezzo': create_checknerror(
            DECIMAL_MAX_TWO_PLACES,
            'Prezzo del prodotto non conforme',
        ...
        'specifica': create_checknerror(
            SINGLE_LINE_STRING, 'Specifica del prodotto non conforme'
        ),
    }
    return _check(checks, inputs)
...
def check_inputs_dettagli_ordine(inputs):
    ...
    checks = (
        CeIdProduttoreRequired
        | {
            'minimo_ordine': create_checknerror(
                DECIMAL_MAX_TWO_PLACES, 'Minimo d\'ordine non conforme'
            )
        }
        | CeNota
    )
    ...
    checks = (
        ...
        | {
            ...
            'importo_rettifica': create_checknerror(
                DECIMAL_MAX_TWO_PLACES, 'Importo rettifica non conforme'
            ),
            'motivazione': create_checknerror(
                MULTI_LINE_STRING, 'Motivazione rettifica non conforme'
            ),
            'rimuovi': create_checknerror(ID, 'Rimuovi non conforme'),
            'pre-conferma': create_checknerror(
                SINGLE_LINE_STRING, 'Conferma non conforme'
            ),
            'conferma': create_checknerror(SINGLE_LINE_STRING, 'Conferma non conforme'),
            'extra_id_utente': create_checknerror(ID, 'ID Utente Extra non conforme'),
            'extra_utente': create_checknerror(
                SINGLE_LINE_STRING, 'Utente Extra non conforme'
            ),
            ...
            'extra_descrizione': create_checknerror(
                MULTI_LINE_STRING, 'Extra descrizione non conforme'
            ),
        }
        | CeDescrizione
    )
...
def check_aggiusta_ordine_chiuso(inputs):
    ...
    checks = (
        {
            'nuovo_totale': create_checknerror(
                DECIMAL_MAX_TWO_PLACES,
...
def check_inputs_contenuto(inputs):
    ...
    checks = {
        'contenuto': create_checknerror(MULTI_LINE_STRING, 'Contenuto non conforme')
    }
