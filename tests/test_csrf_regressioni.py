...
def test_ordini_create_non_fallisce_con_csrf_token(login_moderatore,
                                                   produttore_con_listino):
    client = login_moderatore
    token = get_csrf_token(client, '/ordini/create')

    resp = client.post('/ordini/create', data={
...
def test_ordini_update_non_fallisce_con_csrf_token(login_moderatore,
                                                    produttore_con_listino):
    client = login_moderatore

    # Precondizione: un ordine già aperto da modificare.
    token = get_csrf_token(client, '/ordini/create')
    client.post('/ordini/create', data={
        'id_produttore': produttore_con_listino,
        'scadenza': '2026-08-05',
        'consegna': '2026-08-10',
        'minimo_ordine': '',
    ...
    resp = client.post(
        '/ordini/update/{0}'.format(produttore_con_listino), data={
            'id_produttore': produttore_con_listino,
            'scadenza': '2026-08-06',
            'consegna': '2026-08-11',
            'minimo_ordine': '',
...
def test_listini_create_non_fallisce_con_csrf_token(login_moderatore,
                                                    produttore_con_listino):
    client = login_moderatore
